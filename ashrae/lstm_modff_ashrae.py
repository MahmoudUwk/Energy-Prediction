...
def main() -> None:
    print("=" * 80)
    print("LSTM-ModFF (Modified Firefly) ASHRAE TRAINING - Using best tuned parameters")
    print("=" * 80)
    ...
    units = 187
    num_layers = 1
    learning_rate = 0.006124457871646038
    seq_length = 9
    epochs = 300
    batch_size = 1024
    ...
    # Keep train/val split for validation-based callbacks

    # Model
    input_shape = (X_tr_lstm.shape[1], X_tr_lstm.shape[2])
    print(
        f"Building LSTM-ModFF: input_shape={input_shape}, units={units}, layers={num_layers}, lr={learning_rate}"
    )
    model = Sequential(name="LSTM_ModFF_Final")
    ...
    mape = float(
        100.0 * np.mean(np.abs((y_test_true[mask] - y_test_pred[mask]) / y_test_true[mask]))
    ) if np.any(mask) else float("inf")

    metrics = {"RMSE": rmse, "MAE": mae, "R2": r2, "MAPE": mape}

    print("=" * 80)
    print("LSTM-ModFF RESULTS (Test)")
    print("=" * 80)
    print(metrics)
    print(f"Training time: {train_time_min:.2f} minutes | Test time: {test_time_s:.2f} seconds")

    # Save to lstm_search/artifacts/LSTM-ModFF.obj for plotting
    saver = get_ashrae_results_saver("lstm_search", "LSTM-ModFF")
    ...
    model_info = {
        ...
        "algorithm_name": "Mod_FireflyAlgorithm",
