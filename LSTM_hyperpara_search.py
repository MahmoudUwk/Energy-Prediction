...
class LSTMHyperparameterOptimization(Problem):
    def __init__(self, option,datatype_opt):
    ...
    def _evaluate(self, x):
        ...
        model.fit(X_train, y_train, epochs=1500, batch_size=2**9, verbose=1, shuffle=True, validation_split=0.2,callbacks=callbacks_list)
...
problem = LSTMHyperparameterOptimization(option,datatype_opt)
task = Task(problem, max_evals=30, optimization_type=OptimizationType.MINIMIZATION)
algorithm = Mod_FireflyAlgorithm.Mod_FireflyAlgorithm(population_size = 20)


best_params, best_mse,pop, f_pop = algorithm.run(task)
...
history = model.fit(X_train, y_train, epochs=1500, batch_size=2**9, verbose=1, shuffle=True, validation_split=0.2,callbacks=callbacks_list)

model.set_weights(checkpoint.best_weights)
model.save('LSTMFF'+str(datatype_opt))
#%%
# import keras
# filepath = 'C:/Users/mahmo/OneDrive/Desktop/kuljeet/Energy-Prediction/LSTMFF'
